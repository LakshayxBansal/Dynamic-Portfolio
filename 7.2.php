    ...
    <div class="container w-50">
        ...
        <div class="bg-dark text-white" style="border-radius:10px">
            ...
            <div class="input-group-append">
                <span class="input-group-text"><input class='btn btn-outline-primary btn-block p-0' type='submit' value="Search"  name='search' />
                </span>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea  name="txtaddress" id="" class="form-control" placeholder="" aria-describedby="helpId"><?php if(isset($row)){echo $row[6];}  ?></textarea>
                <small id="helpId" class="text-muted">Help text</small>
            </div>
            <div class="form-group">
                <label for="">Upload Image</label>
                <input type="file" name="fil" class="form-control-file"/>
                <small id="helpId" class="text-muted">Help text</small>
            </div>
            <div class="row">
...
if(isset($_GET["insert"]))
{
   ...
   // echo "$uid $name $password $fullname $email $mobile $address $image";
    $insqry="insert into employee values($uid,'$name','$password','$fullname','$email','$mobile','$address','$image')";
    mysqli_query($conn,$insqry)or die(mysqli_error());
}
...
if(isset($_GET["update"]))
{
    ...
    $image='./image/image(3).jpeg';
   // echo "$uid $name $password $fullname $email $mobile $address $image";
    $updqry="update employee set uname='$name',password='$password',fullname='$fullname',email='$email',mobile='$mobile',address='$address',image='$image' where uid=$uid";
    mysqli_query($conn,$updqry)or die(mysqli_e,rror());
}
if(isset($_GET["delete"]))
{
    $uid=$_GET['txtuid'];
    $delqry="delete from  employee where uid=$uid";
    mysqli_query($conn,$delqry)or die(mysqli_error());
}    
if(isset($_GET["show"]))
{
